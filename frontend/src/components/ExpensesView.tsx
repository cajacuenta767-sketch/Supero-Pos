...
export const ExpensesView: React.FC = () => {
 ...
 activeSubTab === 'expenses' ? 'bg-raised text-accent shadow-e1' : 'text-gray-500'
            }`}
          >
 ...
 className={`px-4 py-2 rounded-md text-body font-bold flex items-center gap-2 transition-all ${
 activeSubTab === 'petty-cash' ? 'bg-raised text-accent shadow-e1' : 'text-gray-500'
            }`}
          >
 ...
 className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-md text-body font-extrabold flex items-center gap-2 shadow-e1 transition-all"
              >
 ...
 return (
                    <tr key={exp.id} className="hover:bg-sunken">
                      <td className="p-4 font-mono font-extrabold text-accent">
                        {exp.id}
                        <span className="block text-micro text-gray-400 font-normal">Ref: {exp.ref_number}</span>
                      </td>
                      <td className="p-4 font-mono text-ink-2">
                        {exp.date}
                      </td>
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-md text-micro font-bold border ${categoryBadge}`}>
                          {exp.category}
                        </span>
                      </td>
                      ...
                      <td className="p-4 text-right font-bold text-ink-2">
                        {exp.user_name}
                      </td>
                      <td className="p-4 text-right space-x-1">
                        <button
 onClick={() => setSelectedExpense(exp)}
 className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-md" title="Ver Comprobante Adjunto"
                        >
                          <Eye className="w-4 h-4" />
                        </button>
                        <button className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-md" title="Imprimir Recibo">
        ...
        <div className="bg-raised rounded-md border border-line p-6 shadow-e1 space-y-6">
          <div className="flex items-center justify-between border-b border-line pb-4">
            <div>
              ...
              <p className="text-body text-gray-500">Administración de recargas y rendimiento de vales auxiliares</p>
            </div>
            <button
 onClick={() => setIsTopUpModalOpen(true)}
 className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md font-bold text-body flex items-center gap-2 shadow"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-5 bg-sunken rounded-md border border-line">
              <span className="text-body text-gray-500 font-bold">Fondo Fijo Asignado</span>
              <p className="font-mono font-black text-display text-accent mt-1">${fixedFund.toFixed(2)}</p>
            </div>

            <div className="p-5 bg-sunken rounded-md border border-line">
              <span className="text-body text-gray-500 font-bold">Egresos Ejecutados en el Turno</span>
              <p className="font-mono font-black text-display text-danger mt-1">-${totalPettyCashSpent.toFixed(2)}</p>
            </div>

            <div className="p-5 bg-ok-soft rounded-md border border-emerald-200 dark:border-emerald-800">
              <span className="text-body text-ok-ink font-bold">Saldo Disponible para Reposición</span>
              <p className="font-mono font-black text-display text-ok mt-1">${availablePettyCash.toFixed(2)}</p>
            </div>
          </div>
        </div>
      )}
 ...
 placeholder="0.00"
 className="w-full mt-1 p-2 bg-sunken border border-line rounded-md font-mono font-bold text-base text-rose-600"
                  />
                </div>
 ...
 className="w-full mt-1 p-2 bg-sunken border border-line rounded-md font-mono text-center font-bold text-base"
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsExpenseModalOpen(false)} className="px-4 py-2 bg-gray-200 bg-sunken text-ink rounded-md font-bold">Cancelar</button>
