...
export const PurchasesView: React.FC = () => {
  ...
  const handleConfirmReceiving = () => {
    ...
    console.log(
      `[CPP & KARDEX] Actualización atómica de CPP e incremento de stock ejecutado para PO ${selectedPO.id}`,
    );
    alert(
      `✅ Recepción de PO ${selectedPO.id} completada al 100%. Stock físico e inventario valorizado (CPP) actualizados.`,
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-raised p-5 rounded-md border border-line shadow-e1">
        <div>
          ...
          <p className="text-body text-ink-2 mt-1">
            Órdenes de compra a proveedores, recepciones con escaneo IMEI y recálculo de Costo
                  ...
                  return (
                    <tr key={po.id} className="hover:bg-sunken">
                      ...
                      <td className="p-4 font-bold text-ink">
                        {po.supplier_name}
                        <span className="block text-micro text-ink-3 font-mono font-normal">
                          NIT: {po.supplier_tax_id}
                        </span>
                      </td>
                      <td className="p-4 font-semibold text-ink-2">{po.branch}</td>
                      <td className="p-4 font-mono font-bold">{po.items_count} productos</td>
                      <td className="p-4 font-mono font-black text-ink">${po.total.toFixed(2)}</td>
      ...
      {activeSubTab === 'receivings' && (
        <div className="bg-raised rounded-md border border-line p-6 shadow-e1 space-y-4">
          <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
            <Truck className="w-5 h-5 text-ok" /> Control de Descarga y Verificación Física en
            Depósito
          </h3>
          <p className="text-body text-ink-3">
            Contraste de guías de remisión del proveedor frente al pedido original
          </p>
          ...
          <div className="p-4 bg-sunken rounded-md border border-line text-body space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-ink-2">
              ...
              <li>
                Para artículos serializados (IMEI), escanear obligatoriamente cada código de barra
                único.
              </li>
              <li>
                Al confirmar la recepción, el sistema recalcula automáticamente el Costo Promedio
                Ponderado (CPP).
              </li>
            </ul>
          </div>
            ...
            <div className="p-5 border-b border-line flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
                  ...
                  <span className="font-mono text-accent">{selectedPO.id}</span>
                </h3>
            ...
            <div className="p-5 space-y-4 text-body">
              ...
              {selectedPO.items.some((i) => i.unit_type === 'SERIALIZED') && (
                <div className="p-4 bg-accent-soft border border-accent/30 dark:border-accent/30 rounded-md space-y-2">
                  <div className="flex items-center justify-between font-extrabold text-accent-ink">
                    ...
                    <span>
                      {scannedImeis.length} de{' '}
                      {selectedPO.items.find((i) => i.unit_type === 'SERIALIZED')?.ordered_qty}{' '}
                      escaneados
                    </span>
                  </div>
                  ...
                  <form onSubmit={handleAddImei} className="flex gap-2">
                    ...
                    <button
                      type="submit"
                      className="px-3 bg-purple-600 text-white rounded-md font-bold"
                    >
                      Agregar
                    </button>
                  </form>
                  ...
                  <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                    {scannedImeis.map((imei) => (
                      <span
                        key={imei}
                        className="px-2 py-0.5 bg-purple-200 text-accent-ink rounded font-mono text-micro font-bold"
                      >
                        {imei}
                      </span>
                    ))}
              ...
              <div className="p-4 bg-ok-soft border border-ok/30 rounded-md space-y-1">
                <span className="font-extrabold text-ok-ink flex items-center gap-1">
                  <TrendingUp className="w-4 h-4" /> Recálculo Automático de Costo Promedio
                  Ponderado (CPP)
                </span>
