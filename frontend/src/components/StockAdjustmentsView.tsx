...
import { Sliders, Search, AlertTriangle, Eye, Printer, Calculator, Lock } from 'lucide-react';
...
export const StockAdjustmentsView: React.FC = () => {
  ...
  const handleApplyAuditReconciliation = () => {
    alert(
      '✅ Ajustes masivos aplicados al inventario. Kardex y valorizaciones de stock actualizadas correctamente.',
    );
                ...
                {filteredAdjustments.map((adj) => {
                  ...
                  return (
                    <tr key={adj.id} className="hover:bg-sunken">
                      <td className="p-4 font-mono font-extrabold text-accent">{adj.id}</td>
                      <td className="p-4 font-mono text-ink-2">{adj.date}</td>
                      <td className="p-4">
                        <span
                          className={`px-2.5 py-1 rounded-md text-micro font-bold border ${typeBadge.color}`}
                        >
                          {typeBadge.label}
                        </span>
                      </td>
                      <td className="p-4 font-semibold text-ink-2">{adj.branch}</td>
                      <td className="p-4 font-mono font-bold">{adj.items_count} ítems</td>
                      <td className="p-4 font-bold text-ink">{adj.user_name}</td>
                      ...
                      <td className="p-4 text-right space-x-1">
                        <button
                          ...
                          title="Ver Conciliación de Diferencias"
                        >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-raised rounded-md border border-line shadow-e3 w-full max-w-2xl overflow-hidden space-y-4">
            <div className="p-5 border-b border-line flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
                  <Calculator className="w-5 h-5 text-accent" /> Sesión de Conteo Físico Ciego de
                  Inventario
                </h3>
                <p className="text-body text-ink-3">
                  Sucursal: {auditBranch} (El stock teórico permanece oculto)
                </p>
              </div>
              <button
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-raised rounded-md border border-line shadow-e3 w-full max-w-3xl overflow-hidden space-y-4">
            <div className="p-5 border-b border-line flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
                  <Calculator className="w-5 h-5 text-ok" /> Reporte de Conciliación & Desviaciones
                  de Stock
                </h3>
                <p className="text-body text-ink-3">
                  Cruce automático entre Stock Teórico vs Stock Físico Contado
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-5 space-y-4 text-body">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  onClick={handleApplyAuditReconciliation}
