  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-[#000000] h-[calc(100vh-56px)] overflow-y-auto pr-2 space-y-6 select-none transition-colors duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#121212] p-5 rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-sm">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2">
            ...
            8. Ajuste de Stock & Auditoría de Inventario
          </h1>
        ...
        <div className="flex items-center bg-gray-100 dark:bg-[#0B0C10] p-1.5 rounded-xl border border-gray-200 dark:border-[#1F2833]">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${
              activeSubTab === 'adjustments' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
          ...
          <button
            ...
            className="px-3 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow"
          >
          ...
          <button
            ...
            className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow"
          >
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-[#1F2833] text-xs">
                {filteredAdjustments.map((adj) => {
                  ...
                  return (
                    <tr key={adj.id} className="hover:bg-gray-50 dark:hover:bg-[#1A1D20]">
                      ...
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border ${typeBadge.color}`}>
                          {typeBadge.label}
                        </span>
                      </td>
                      ...
                      <td className="p-4 text-center">
                        <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950 border border-emerald-200">
                          APLICADO
                        </span>
                      </td>
                      <td className="p-4 text-right space-x-1">
                        <button
                          ...
                          className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Ver Conciliación de Diferencias"
                        >
                          <Eye className="w-4 h-4" />
                        </button>
                        <button className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Imprimir Reporte Kardex">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-2xl w-full max-w-md overflow-hidden space-y-4">
            ...
            <form onSubmit={handleConfirmLoss} className="p-5 space-y-4 text-xs">
              <div>
                <label className="font-bold text-gray-700 dark:text-gray-300">Causa / Motivo Operativo *</label>
                <select
                  value={lossReason}
              ...
              <div>
                <label className="font-bold text-gray-700 dark:text-gray-300">Descripción del Incidente *</label>
                <textarea
                  required rows={2}
                  value={lossNotes}
                  onChange={(e) => setLossNotes(e.target.value)}
                  placeholder="Detallar causa exacta de la baja de mercadería..."
                  className="w-full mt-1 p-2 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-xl"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsLossModalOpen(false)} className="px-4 py-2 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-xl font-bold">Cancelar</button>
          ...
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-2xl w-full max-w-2xl overflow-hidden space-y-4">
            <div className="p-5 border-b border-gray-100 dark:border-[#1F2833] flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">Sucursal: {auditBranch} (El stock teórico permanece oculto)</p>
              </div>
            ...
            <div className="p-5 space-y-4 text-xs">
                      ...
                      <tr key={item.id}>
                        ...
                        <td className="p-3 text-center">
                          <input
                            ...
                            className="w-20 p-1 bg-blue-50 dark:bg-blue-950/60 border border-blue-300 rounded text-center font-mono font-black text-sm text-blue-600"
                          />
                        </td>
                      </tr>
                    ))}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsAuditModalOpen(false)} className="px-4 py-2 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-xl font-bold">Cancelar</button>
                <button
                  onClick={() => {
                    setIsAuditModalOpen(false);
                    setIsReconciliationModalOpen(true);
                  }}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-extrabold shadow"
                >
      ...
      {(isReconciliationModalOpen || selectedRecord) && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-2xl w-full max-w-3xl overflow-hidden space-y-4">
            <div className="p-5 border-b border-gray-100 dark:border-[#1F2833] flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-base text-gray-900 dark:text-white flex items-center gap-2">
                  <Calculator className="w-5 h-5 text-emerald-500" /> Reporte de Conciliación & Desviaciones de Stock
                </h3>
                <p className="text-xs text-gray-500">Cruce automático entre Stock Teórico vs Stock Físico Contado</p>
              </div>
            ...
            <div className="p-5 space-y-4 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => { setIsReconciliationModalOpen(false); setSelectedRecord(null); }} className="px-4 py-2 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-xl font-bold">Cerrar</button>
                <button onClick={handleApplyAuditReconciliation} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-extrabold shadow">
