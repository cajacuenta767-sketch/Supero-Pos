  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-[#000000] h-[calc(100vh-56px)] overflow-y-auto pr-2 space-y-6 select-none transition-colors duration-200">
        ...
        <div className="flex items-center bg-gray-100 dark:bg-[#0B0C10] p-1.5 rounded-xl border border-gray-200 dark:border-[#1F2833]">
          <button
            ...
            className={`px-3.5 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${
              activeSubTab === 'timeclock' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${
              activeSubTab === 'shifts' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${
              activeSubTab === 'engine' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${
              activeSubTab === 'exceptions' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
      ...
      {activeSubTab === 'timeclock' && (
        <div className="max-w-xl mx-auto bg-white dark:bg-[#121212] p-8 rounded-3xl border border-gray-200 dark:border-[#1F2833] shadow-2xl space-y-6 text-center">
          <div>
            <div className="w-16 h-16 bg-blue-100 dark:bg-blue-950 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center mx-auto mb-3">
              <Clock className="w-8 h-8" />
            </div>
            <h2 className="text-xl font-black text-gray-900 dark:text-white">Marcación de Asistencia en Terminal POS</h2>
            <p className="text-xs text-gray-500 mt-1">Ingrese su PIN personal de 4 a 6 dígitos para validar entrada/salida</p>
          </div>
          ...
          <form onSubmit={handleClockPUNCH} className="space-y-6">
            <div className="grid grid-cols-2 gap-3 text-xs font-bold">
              <button
                ...
                className={`p-3.5 rounded-2xl border flex items-center justify-center gap-2 transition-all ${
                  clockEventType === 'CLOCK_IN' ? 'bg-emerald-600 text-white border-emerald-600 shadow-lg' : 'bg-gray-100 dark:bg-[#0B0C10] text-gray-700 dark:text-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`p-3.5 rounded-2xl border flex items-center justify-center gap-2 transition-all ${
                  clockEventType === 'BREAK_START' ? 'bg-amber-600 text-white border-amber-600 shadow-lg' : 'bg-gray-100 dark:bg-[#0B0C10] text-gray-700 dark:text-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`p-3.5 rounded-2xl border flex items-center justify-center gap-2 transition-all ${
                  clockEventType === 'BREAK_END' ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'bg-gray-100 dark:bg-[#0B0C10] text-gray-700 dark:text-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`p-3.5 rounded-2xl border flex items-center justify-center gap-2 transition-all ${
                  clockEventType === 'CLOCK_OUT' ? 'bg-rose-600 text-white border-rose-600 shadow-lg' : 'bg-gray-100 dark:bg-[#0B0C10] text-gray-700 dark:text-gray-300'
                }`}
              >
            ...
            <div className="space-y-2">
              <label className="font-bold text-xs text-gray-700 dark:text-gray-300 block">PIN Personal (4 a 6 dígitos) *</label>
              <input
                type="password"
                required
                maxLength={6}
                ...
                placeholder="• • • •"
                className="w-full p-4 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-2xl font-mono text-center font-black text-2xl tracking-widest text-gray-900 dark:text-white"
              />
            </div>

            <div className="p-3 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 rounded-xl text-left text-[11px] text-blue-700 dark:text-blue-300 font-semibold">
              🔒 Regla de Seguridad: La terminal POS requiere registro oficial de ENTRADA para abrir la caja física.
            </div>

            <button
              type="submit"
              className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black text-sm shadow-xl flex items-center justify-center gap-2"
            >
      ...
      {activeSubTab === 'shifts' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between bg-white dark:bg-[#121212] p-4 rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-sm">
            <h3 className="font-extrabold text-base text-gray-900 dark:text-white">Programación de Turnos & Tolerancia de Ingreso</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {shifts.map(s => (
              <div key={s.id} className="bg-white dark:bg-[#121212] p-6 rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-sm space-y-3">
                <div className="flex items-center justify-between border-b border-gray-100 dark:border-[#1F2833] pb-3">
                  <h4 className="font-extrabold text-base text-gray-900 dark:text-white flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-blue-500" /> {s.name}
                  </h4>
                  <span className="px-2.5 py-1 bg-blue-100 text-blue-800 dark:bg-blue-950 border border-blue-200 rounded-lg text-xs font-mono font-bold">
                    {s.start_time} - {s.end_time}
                  </span>
                </div>

                <div className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
                  <p className="font-semibold">Tiempo de Gracia Tolerado: <strong className="text-gray-900 dark:text-white font-mono">{s.grace_period_mins} minutos</strong></p>
                  <p className="font-semibold">Personal Asignado: <strong className="text-gray-900 dark:text-white">{s.assigned_employees.join(', ')}</strong></p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#121212] p-4 rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-sm">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-xl text-xs text-gray-900 dark:text-white font-semibold"
              />
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-[#1F2833] text-xs">
                {filteredLogs.map((log) => {
                  const statusBadge = {
                    ON_TIME: { label: 'A TIEMPO (PUNTUAL)', color: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 border-emerald-200' },
                    LATE: { label: `RETRASO (${log.lateness_minutes} min)`, color: 'bg-rose-100 text-rose-800 dark:bg-rose-950 border-rose-200' },
                    OVERTIME: { label: 'HORAS EXTRAS', color: 'bg-purple-100 text-purple-800 dark:bg-purple-950 border-purple-200' },
                    JUSTIFIED: { label: 'JUSTIFICADO', color: 'bg-blue-100 text-blue-800 dark:bg-blue-950 border-blue-200' },
                  }[log.status];

                  return (
                    <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-[#1A1D20]">
                      <td className="p-4 font-mono font-extrabold text-blue-600 dark:text-blue-400">{log.id}</td>
                      <td className="p-4 font-mono text-gray-600 dark:text-gray-300">{log.timestamp}</td>
                      <td className="p-4 font-bold text-gray-900 dark:text-white">{log.employee_name} ({log.role})</td>
                      <td className="p-4 font-bold">
                        <span className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded font-mono text-[10px]">
                          {log.event_type}
                        </span>
                      </td>
                      <td className="p-4 text-center font-mono font-black text-sm text-gray-900 dark:text-white">
      ...
      {activeSubTab === 'exceptions' && (
        <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-gray-100 dark:border-[#1F2833] pb-4">
            <div>
              <h3 className="font-extrabold text-base text-gray-900 dark:text-white flex items-center gap-2">
                <FileText className="w-5 h-5 text-blue-500" /> Licencias Médicas, Permisos & Justificación de Faltas
              </h3>
              <p className="text-xs text-gray-500">Registro de excepciones con o sin goce de haber</p>
            </div>
            <button
              onClick={() => setIsExceptionModalOpen(true)}
              className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs flex items-center gap-2 shadow"
            >
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-gray-100 dark:bg-[#0B0C10] text-gray-500 uppercase text-[11px] font-bold border-b border-gray-200 dark:border-[#1F2833]">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-[#1F2833]">
              {exceptions.map(exc => (
                <tr key={exc.id}>
                  <td className="p-4 font-bold text-gray-900 dark:text-white">{exc.employee_name}</td>
                  <td className="p-4 font-semibold text-blue-600 dark:text-blue-400">{exc.type}</td>
                  <td className="p-4 font-mono text-gray-600 dark:text-gray-300">{exc.date_range}</td>
                  <td className="p-4 font-semibold text-gray-700 dark:text-gray-300">{exc.notes}</td>
                  <td className="p-4 text-right">
                    <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 dark:bg-emerald-950 border border-emerald-200 rounded-lg text-[10px] font-bold">
                      APROBADO
                    </span>
                  </td>
                </tr>
