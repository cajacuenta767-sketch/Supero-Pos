...
export const SettingsView: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'company' | 'hardware' | 'security' | 'sync'>(
    'company',
  );
    ...
    <div className="p-6 bg-canvas h-[calc(100vh-56px)] overflow-y-auto pr-2 space-y-6 select-none transition-colors duration-fast ease-ease">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-raised p-5 rounded-md border border-line shadow-e1">
        <div>
          ...
          <p className="text-body text-ink-2 mt-1">
            Configuración institucional, periféricos POS, políticas de seguridad RBAC y motor de
      ...
      {activeSubTab === 'company' && (
        <div className="space-y-6 max-w-4xl">
          <form
            ...
            className="bg-raised p-6 rounded-md border border-line shadow-e1 space-y-6"
          >
            <div className="flex items-center justify-between border-b border-line pb-4">
              <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
                <Building2 className="w-5 h-5 text-accent" /> Identidad Corporativa y Configuración
                Fiscal
              </h3>
              <button
                type="submit"
                className="px-4 py-2 bg-accent hover:bg-accent-hover text-white rounded-md text-body font-extrabold flex items-center gap-1.5 shadow"
              >
            ...
            <div className="flex items-center gap-6 p-4 bg-sunken rounded-md border border-line">
              ...
              <div className="space-y-1">
                <span className="font-extrabold text-body text-ink block">
                  Logotipo Oficial Institucional
                </span>
                <p className="text-micro text-ink-3">
                  Se imprimirá en la cabecera de comprobantes térmicos y facturas PDF (PNG/JPG máx
                  2MB)
                </p>
          ...
          <div className="bg-raised p-6 rounded-md border border-line shadow-e1 space-y-4">
            <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
              <Building2 className="w-5 h-5 text-ok" /> Directorio de Sucursales y Almacenes
              Vinculados
            </h3>
      ...
      {activeSubTab === 'hardware' && (
        <div className="bg-raised p-6 rounded-md border border-line shadow-e1 space-y-6 max-w-3xl">
          <div className="flex items-center justify-between border-b border-line pb-4">
            <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
              <Printer className="w-5 h-5 text-accent" /> Periféricos POS: Impresora, Gaveta &
              Balanza Electrónica
            </h3>
            <button
              onClick={() => alert('🖨️ Imprimiendo ticket de calibración hardware...')}
              className="px-4 py-2 bg-ok hover:opacity-90 text-white rounded-md text-body font-bold flex items-center gap-1.5 shadow"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-body">
            ...
            <div className="md:col-span-2 space-y-3 pt-2">
              ...
              <label className="flex items-center gap-2 font-bold cursor-pointer">
                ...
                />
                <span>
                  Enviar pulso eléctrico a gaveta portamonedas al liquidar venta en efectivo
                </span>
              </label>
            </div>
          </div>
      ...
      {activeSubTab === 'security' && (
        <div className="bg-raised p-6 rounded-md border border-line shadow-e1 space-y-6 max-w-3xl">
          <div className="border-b border-line pb-4">
            <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
              <ShieldAlert className="w-5 h-5 text-danger" /> Políticas de Seguridad Operativa &
              Reglas RBAC
            </h3>
          </div>
          ...
          <div className="space-y-4 text-body">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="font-bold text-ink-2 flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-warn" /> Inactividad para Bloqueo de Pantalla
                  (Minutos)
                </label>
                <input
                  type="number"
      ...
      {activeSubTab === 'sync' && (
        <div className="bg-raised p-6 rounded-md border border-line shadow-e1 space-y-6 max-w-3xl">
          <div className="flex items-center justify-between border-b border-line pb-4">
            <h3 className="font-extrabold text-base text-ink flex items-center gap-2">
              <RefreshCw className="w-5 h-5 text-ok" /> Arquitectura Offline-First & Cola SQLite
              (sync_queue)
            </h3>
            <button
              onClick={() =>
                alert('⚡ Bucle de sincronización bidireccional ejecutado en segundo plano.')
              }
              className="px-4 py-2 bg-accent hover:bg-accent-hover text-white rounded-md text-body font-bold flex items-center gap-1.5 shadow"
            >
          ...
          <div className="space-y-4 text-body">
            <div className="p-4 bg-ok-soft border border-ok/30 rounded-md space-y-1">
              <span className="font-extrabold text-ok-ink flex items-center gap-1">
                <CheckCircle2 className="w-4 h-4" /> Persistencia Local Autónoma SQLite
                (better-sqlite3)
              </span>
              <p className="text-micro text-ink-2">
                La terminal opera sin interrupciones ante cortes de red. Las ventas y stock se
                almacenan en <code>sync_queue</code> y se transmiten al detectar señal.
              </p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="font-bold text-ink-2">
                  Frecuencia del Worker Loop (Segundos) *
                </label>
                <input
                  type="number"
            ...
            <div className="p-4 bg-sunken rounded-md border border-line flex items-center justify-between">
              <div>
                ...
                <span className="text-ink-3 text-micro">
                  Alternar entre Light Mode (#FFFFFF) y Dark Mode (#000000)
                </span>
              </div>
              <button
                onClick={toggleTheme}
