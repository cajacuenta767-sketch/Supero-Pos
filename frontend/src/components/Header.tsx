...
export const Header: React.FC<HeaderProps> = ({ searchQuery, setSearchQuery }) => {
  ...
  return (
    <header
      className={cn(
        'h-14 shrink-0 bg-surface border-b px-4 flex items-center gap-2 sm:gap-3 select-none',
    ...
    >
      ...
      <div className="relative flex-1 min-w-0 max-w-xl">
        ...
        <input
          ...
          className="w-full h-9 pl-9 pr-3 lg:pr-20 bg-sunken border border-line rounded-md text-base text-ink hover:border-line-strong focus:border-accent transition-colors duration-fast ease-ease"
        />
        <span className="hidden lg:block absolute right-2.5 top-1/2 -translate-y-1/2">
          <Kbd keys={['⌘K', 'F2']} />
        </span>
      </div>
      ...
      <select
        ...
        className="hidden md:block h-9 px-2.5 bg-sunken border border-line rounded-md text-body font-semibold text-ink cursor-pointer hover:border-line-strong transition-colors duration-fast ease-ease"
      >
