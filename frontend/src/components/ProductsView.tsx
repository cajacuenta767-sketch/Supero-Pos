    ...
    <div className="p-6 bg-gray-50 dark:bg-[#000000] h-[calc(100vh-56px)] overflow-y-auto pr-2 space-y-6 select-none transition-colors duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#121212] p-5 rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-sm">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2">
            ...
            4. Productos y Catálogo Maestro
          </h1>
        ...
        <div className="flex items-center bg-gray-100 dark:bg-[#0B0C10] p-1.5 rounded-xl border border-gray-200 dark:border-[#1F2833]">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${
              activeTab === 'catalog' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${
              activeTab === 'serials' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${
              activeTab === 'categories' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${
              activeTab === 'labels' ? 'bg-white dark:bg-[#121212] text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500'
            }`}
          >
      ...
      {activeTab === 'catalog' && (
        <div className="space-y-4">
          {/* Top Toolbar */}
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white dark:bg-[#121212] p-4 rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-sm">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-[#1F2833] text-xs">
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-[#1A1D20] transition-colors">
                      <td className="p-4 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] flex items-center justify-center font-bold text-gray-400">
                        ...
                        <div>
                          ...
                          <span className="text-gray-400 font-mono text-[11px] block">{p.sku} • EAN: {p.barcode}</span>
                        </div>
                      </td>
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border flex items-center gap-1.5 w-fit ${unitBadge.color}`}>
                          {unitBadge.icon}
                          {unitBadge.label}
                        </span>
                      </td>
                      <td className="p-4 font-mono">
                        <div className="text-[11px]">
                          <span className="text-gray-400">Costo: <span className="font-bold text-gray-700 dark:text-gray-300">${p.cost_price.toFixed(2)}</span></span>
                          <span className="mx-1">•</span>
                          <span className="text-blue-600 dark:text-blue-400 font-extrabold">P.Venta: ${p.sale_price.toFixed(2)}</span>
                        </div>
                      ...
                      <td className="p-4 font-mono font-bold">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs ${
                          isLowStock ? 'bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300 animate-pulse' : 'text-gray-900 dark:text-white'
                        }`}>
                      ...
                      <td className="p-4 text-center">
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                            p.is_active ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-700'
                          }`}
                        >
                      ...
                      <td className="p-4 text-right space-x-1">
                        {canEditProduct && (
                          <button
                            ...
                            className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Editar Producto"
                          >
                            <Edit3 className="w-4 h-4" />
                          </button>
                        )}
                        <button className="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg" title="Duplicar">
                          <Copy className="w-4 h-4" />
                        </button>
                        <button className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg" title="Kardex Individual">
            ...
            <div className="p-4 bg-gray-50 dark:bg-[#0B0C10] border-t border-gray-200 dark:border-[#1F2833] flex items-center justify-between text-xs text-gray-500">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="px-2 py-1 bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#1F2833] rounded-lg text-xs font-semibold"
                >
        ...
        <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] p-6 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 dark:border-[#1F2833] pb-4">
            <div>
              ...
              <p className="text-xs text-gray-500">Búsqueda rápida y estado de garantía de equipos con número de serie único</p>
            </div>
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-xl text-xs font-mono"
              />
            </div>
          </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-[#1F2833] text-xs font-mono">
              {serials.filter(s => s.serial_number.includes(serialSearch) || s.product_name.toLowerCase().includes(serialSearch.toLowerCase())).map((s) => (
                <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-[#1A1D20]">
                  <td className="p-4 font-bold text-gray-900 dark:text-white font-sans">{s.product_name}</td>
                  <td className="p-4 font-bold text-purple-600 dark:text-purple-400">{s.serial_number}</td>
                  ...
                  <td className="p-4 font-sans text-gray-600 dark:text-gray-300">
                    {s.ticket_id ? `${s.ticket_id} • ${s.customer_name}` : 'Sin asignar (En Tienda)'}
                  </td>
                  <td className="p-4 text-right text-gray-400 text-[11px]">{s.updated_at}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-[#1F2833] pb-3">
              <h3 className="font-extrabold text-base text-gray-900 dark:text-white">Categorías Comerciales</h3>
              <button className="px-3 py-1.5 bg-blue-600 text-white font-bold text-xs rounded-xl flex items-center gap-1">
            ...
            <div className="space-y-2 text-xs">
              {['Bebidas & Gaseosas', 'Lácteos & Fiambrería', 'Electrónica & Celulares', 'Golosinas & Snacks'].map((cat, idx) => (
                <div key={idx} className="p-3 bg-gray-50 dark:bg-[#0B0C10] rounded-xl border border-gray-200 dark:border-[#1F2833] flex items-center justify-between font-bold">
                  <span>{cat}</span>
                  <span className="text-gray-400 font-mono text-[11px]">Activa</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-[#1F2833] pb-3">
              <h3 className="font-extrabold text-base text-gray-900 dark:text-white">Unidades de Medida y Conversiones</h3>
              <button className="px-3 py-1.5 bg-blue-600 text-white font-bold text-xs rounded-xl">+ Nueva Unidad</button>
            </div>
            <div className="space-y-2 text-xs">
              <div className="p-3 bg-gray-50 dark:bg-[#0B0C10] rounded-xl border border-gray-200 dark:border-[#1F2833] flex items-center justify-between">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="font-bold text-gray-700 dark:text-gray-300">Seleccionar Producto</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold text-gray-700 dark:text-gray-300">Formato de Etiqueta</label>
                ...
                <div>
                  <label className="font-bold text-gray-700 dark:text-gray-300">Cantidad a Imprimir</label>
                  <input type="number" defaultValue={24} className="w-full mt-1 p-2 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-xl font-mono font-bold" />
                </div>
              </div>

              <button className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-extrabold flex items-center justify-center gap-2 shadow-lg transition-all">
          ...
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] p-6 shadow-sm space-y-4 flex flex-col items-center justify-center">
            <span className="text-xs font-extrabold text-gray-400 uppercase tracking-widest">Vista Previa de Etiqueta Térmica</span>
            
            {/* Thermal Label Mockup */}
            <div className="w-64 p-4 bg-white text-black border-2 border-dashed border-gray-400 rounded-lg text-center space-y-2 shadow-md">
              <span className="font-black text-xs block tracking-wide uppercase">SUPERO POS ENTERPRISE</span>
              <p className="font-bold text-xs line-clamp-1">{products[0]?.name}</p>
              <div className="py-2 bg-black text-white font-mono text-[10px] font-bold tracking-widest rounded">
                ||||| | |||||| |||| | |||||
              </div>
              <span className="font-mono text-[11px] block">{products[0]?.barcode}</span>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-[#1F2833] shadow-2xl w-full max-w-2xl overflow-hidden space-y-4">
            ...
            <form onSubmit={handleSaveProduct} className="p-5 space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold text-gray-700 dark:text-gray-300">Código SKU *</label>
                  <div className="flex gap-2 mt-1">
                    <input
                      type="text" required value={formData.sku}
                      onChange={(e) => setFormData({ ...formData, sku: e.target.value })}
                      placeholder="SKU-1001"
                      className="w-full p-2 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-xl font-mono font-bold"
                    />
                    <button type="button" onClick={generateSKU} className="px-3 bg-blue-50 text-blue-600 font-bold rounded-xl text-[11px]" title="Generar SKU">Auto</button>
                  </div>
                </div>

                <div>
                  <label className="font-bold text-gray-700 dark:text-gray-300">Código de Barras (EAN-13)</label>
                  <div className="flex gap-2 mt-1">
                    <input
                      type="text" value={formData.barcode}
                      onChange={(e) => setFormData({ ...formData, barcode: e.target.value })}
                      placeholder="7771234567890"
                      className="w-full p-2 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-xl font-mono"
                    />
                    <button type="button" onClick={generateEAN13} className="px-3 bg-emerald-50 text-emerald-600 font-bold rounded-xl text-[11px]" title="Generar EAN13">EAN</button>
                  </div>
                </div>
              </div>

              <div>
                <label className="font-bold text-gray-700 dark:text-gray-300">Nombre Comercial del Producto *</label>
                <input
                  type="text" required value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  placeholder="Ej. Coca Cola 2 Litros Retornable"
                  className="w-full mt-1 p-2 bg-gray-100 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#1F2833] rounded-xl font-semibold"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold text-gray-700 dark:text-gray-300">Tipo de Unidad *</label>
                  <select
              ...
              <div className="p-3 bg-gray-50 dark:bg-[#0B0C10] rounded-xl border border-gray-200 dark:border-[#1F2833] space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-extrabold text-gray-900 dark:text-white">Precios & Margen de Rentabilidad (CPP)</span>
                  <span className="text-xs font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950 px-2 py-0.5 rounded-lg border border-emerald-200">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="font-bold text-gray-600 dark:text-gray-400 text-[11px]">Costo Base (CPP) $</label>
                    <input
                      type="number" step="0.1" value={formData.cost_price}
                      onChange={(e) => setFormData({ ...formData, cost_price: Number(e.target.value) })}
                      className="w-full mt-1 p-2 bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#1F2833] rounded-xl font-mono font-bold"
                    />
                  </div>
                  <div>
                    <label className="font-bold text-blue-600 dark:text-blue-400 text-[11px]">Precio Minorista $</label>
                    <input
                      type="number" step="0.1" value={formData.sale_price}
                      onChange={(e) => setFormData({ ...formData, sale_price: Number(e.target.value) })}
                      className="w-full mt-1 p-2 bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#1F2833] rounded-xl font-mono font-extrabold text-blue-600"
                    />
                  </div>
                  <div>
                    <label className="font-bold text-emerald-600 dark:text-emerald-400 text-[11px]">Precio Mayorista $</label>
                    <input
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsProductModalOpen(false)} className="px-4 py-2 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-xl font-bold">Cancelar</button>
                <button type="submit" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-extrabold shadow">Guardar Producto</button>
              </div>
            </form>
          </div>
        </div>
